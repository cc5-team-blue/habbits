...
export class Main extends Component {
  // wip: get JournalHabbit status from firebase
  componentDidMount() {
    const { getJournalStatus } = this.props;
    getJournalStatus();
  }

  render() {
    const { clickHabbit, goToAnalytics, achievements, goToJournal } = this.props;
    return (
...
const mapStateToProps = state => ({
  ...
  // wip: set journalData from state
  isJournalActive: state.journalData,
});
...
const mapDispatchToProps = dispatch => ({
  ...
  // wip: getJournalStatus from firebase
  getJournalStatus: () => {
    firebase
      .database()
      ...
      .on('value', data => {
        ...
        dispatch(getJournalData(data.val()));
      });
  },
});
